import json
import re
from .agents import Qwen_VL
import datetime
import os
# from methods.utils import setup_model,qwen_vl_generate_content,qwen_vl_chat_content
...
class LLM_Debate(Discussion):
    def __init__(self, agents_config, dataset_file, rounds, task, prompt,root_path=None, batch_manager=None):
        super().__init__(dataset_file, rounds, prompt)
        self.task_type = task
        self.batch_manager=batch_manager
        self.agents = self.initialize_agents(agents_config,root_path,self.batch_manager)
        print(f"Debate initialized for task: {task} with {len(self.agents)} agents.")
    
    def initialize_agents(self, agents_config,root_path, batch_manager=None,video_batch_manager=None):
        agents = []
        for config in agents_config:
            # if "Qwen" in config['model_name'] or "LLaVA" in config['model_name']:
            agents.append(Qwen_VL(model_name=config['model_name'],
