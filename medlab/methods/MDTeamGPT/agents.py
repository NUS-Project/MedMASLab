...
class MDTAgents:
    def __init__(self, api_key_vl=None, base_url_vl=None,api_key_text=None, base_url_text=None,  text_model=None, vl_model=None, enable_tools=True,root_path=None,batch_manager=None):
    ...
    def primary_care_doctor(self, case_info: str) -> Dict[str, Any]:
        ...
        content,prompt_tokens,completion_tokens=qwen_generate_content([qwen_vl_chat_content(image_path=None,message=msg)],self.batch_manager)
    ...
    def specialist_consult(self, role: str, case_info: str, residual_context: str,
        ...
        if self.tools.enable:
            try:
                ...
                kw,prompt_tokens,completion_tokens=qwen_generate_content([qwen_vl_chat_content(image_path=image_data,message=msg)],self.batch_manager)
...
# （陈述您明确的医疗意见或诊断结果。）"""
    ...
    def cot_reviewer(self, case_info, final_answer, ground_truth):
        ...
        # if "Qwen" in self.text_model or "LLaVA" in self.text_model:
        try:
            content,prompt_tokens,completion_tokens=qwen_vl_generate_content(self.critic_model,self.critic_processor,[qwen_vl_chat_content(image_path=None,message=msg)])
            self.token_stats["MDTeamGPT"]["num_llm_calls"] += 1
            self.token_stats["MDTeamGPT"]["prompt_tokens"] += prompt_tokens
            self.token_stats["MDTeamGPT"]["completion_tokens"] += completion_tokens
            content = content.strip()
